...
export const ShareVia = ({
	shareUrl,
	description,
}: { shareUrl: string; description?: string }) => {
	const handleLinkedInShare = () => {
		const descriptionWithAmpersands = description?.replace(/\s+/g, "&");
		const linkedInShareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(
			shareUrl,
		)}&shareActive=true&text=${
			encodeURIComponent(descriptionWithAmpersands!) ?? "Just sharing this link"
		}`;
	...
	const handleEmailShare = () => {
		...
		const emailBody = encodeURIComponent(
			`I thought you might find this interesting: ${shareUrl} \n ${description}`,
		);
